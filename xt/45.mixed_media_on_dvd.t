...
$media->queue_media( $input_image );
ok( $media->queue_count() == 0, 'queue is still empty' );
my $drno_conf < io 'xt/conf/dr_no.mixed.conf';
...
ok( readlink( 'xt/movies/Year/1962/Dr. No - PG (1962).m4v' ) 
    eq '../../All/Dr. No - PG (1962)/Dr. No - PG (1962).m4v',
        'Year 1962 symlink' );
...
$media->encode_media( $payload );
$job->finish();
$target_file = 'xt/tv/Dr. No Extras/Trailer 1.m4v';
ok( -f $target_file, 'file installed' );
...
$media->encode_media( $payload );
$job->finish();
$target_file = 'xt/tv/Dr. No Extras/Trailer 2.m4v';
ok( -f $target_file, 'file installed' );
